const ERC1132 = artifacts.require('ERC1132');
...
contract('ERC1132', ([owner, receiver]) => {
  const name = 'My Token';
  const symbol = 'MTKN';

  before(async () => {
    token = await ERC1132.new(name, symbol);
  });
  ...
  it('can unLockTokens', async () => {
    ...
    let unERC1132 = await token.getUnERC1132s(owner);
    assert.equal(unERC1132.toNumber(), tokensLocked.toNumber());
    await token.unlock(owner);
    unERC1132 = await token.getUnERC1132s(owner);
    assert.equal(unERC1132.toNumber(), 0);
